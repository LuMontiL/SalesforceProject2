...
import TRAIL_DETAIL_FIELD from '@salesforce/schema/Trail__c.Detalle__c';
import TRAIL_POINTS_FIELD from '@salesforce/schema/Trail__c.Puntos__c';
import TRAIL_ESTIMATED_TIME_FIELD from '@salesforce/schema/Trail__c.Tiempo_Estimado__c';
import { CurrentPageReference } from 'lightning/navigation';


const TRAIL_OBJECT = 'Trail__c';
const TRAIL_FIELDS = [TRAIL_ID_FIELD, TRAIL_NAME_FIELD, TRAIL_DETAIL_FIELD, TRAIL_POINTS_FIELD, TRAIL_ESTIMATED_TIME_FIELD]
...
export default class TrailView extends NavigationMixin(LightningElement) {
    ...
    showUnitChecks(){
        for (let i = 0; i < this.data.modules.length; i++) {
            for (let j = 0; j < this.data.modules[i].Unidades__r.length; j++) {
                if(this.data.passedUnitIds.includes(this.data.modules[i].Unidades__r[j].Id)){
                    let unitId = this.data.modules[i].Unidades__r[j].Id;
